...
4. **Set two variables.** Service → *Variables* → add `SESSION_SECRET` = any long random string (e.g. from https://generate-secret.vercel.app/32), and `SETUP_CODE` = a code only you know. The setup code is required to create the owner account on an empty database — without it, anyone who opens your URL while the database is empty (fresh deploy, missing volume) could claim the app as theirs.
...
Insights include a ten-second plain-language summary (updated on every visit), your week ranked by day, channel growth/decline, cost categories creeping faster than revenue, weekly margin and effective commission rate series, unusual days flagged against weekday norms, best/worst records, and scheduled labor vs revenue. Extras: a what-if simulator (price ±%, food cost ±%, extra recurring cost — instant margin/break-even effect on a copy of this month, real data untouched), monthly profit/margin goals with pace tracking, month-to-date location comparison (2+ locations), and upcoming Mexican holiday notes.
...
## Team schedule
Scheduled labor books itself into your P&L automatically, day by day: hourly staff cost lands on the days they work, and weekly salaries are spread evenly across any week that has a schedule. It flows into profit, break-even, the trend chart, the labor benchmark, insights, and forecasts (future weeks you've already scheduled use their real cost; unscheduled weeks assume your most recent scheduled week). If you also keep payroll as a recurring cost, the app warns you about double-counting — keep the recurring item only as the budget line, or delete it and let the schedule be the source of truth.
